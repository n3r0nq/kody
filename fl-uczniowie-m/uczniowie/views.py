...
def lista_kl():
    klasa = Klasa.select()
    return render_template('lista_kl.html', query=klasa)
...
@app.route('/uczen_usun/<int:kid>', methods=['GET', 'POST'])
def uczen_usun(kid):
    """Usuwanie klase o podanym id"""
    k = get_or_404(Uczen, kid)
    if request.method == 'POST':
        flash('Usunięto klase {}'.format(k.imie), 'sukces')
        for o in Uczen.select().where(Uczen.imie == k.id):
            o.delete_instance()
        k.delete_instance()
        return redirect(url_for('lista_ucz'))
    return render_template('uczen_usun.html', klasa=k)
